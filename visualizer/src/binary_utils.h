...
#define LIDAR_ID 0
#define SAVE_TIME 10000     //180000 // 3분(ms)
...
public:
    ...
    // binary search with object
    int findClosestObjectFrame(uint64_t lidar_time, const std::vector<ObjBinary>& obj_list);

    // void mergeDataByTime(const std::vector<LidarBinary>& lidar_list, const std::vector<ObjBinary>& obj_list);
